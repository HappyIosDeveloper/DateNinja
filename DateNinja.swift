...
/*
 USAGE
 print(String(describing: "2017-10-20 15:40:00".toDateNinja())) // GMT
...
extension String {
    ...
    func toDateNinja(_ resetTime: Bool = false)-> Date {
        let stringWithoutPlus = self.removeAfter("+")
        let stringWithoutDot = stringWithoutPlus.removeAfter(".")
        var std = stringWithoutDot.replace("T", with:" ").replace("/", with: "-") // for dates like this: 2020/10/10
        ...
        print(" *********** ==> COULC NOT CONVERT THIS DATE: \(self) <== *********** ")
        dateFormatterGet.dateFormat = "yyyy-MM-dd hh:mm:ss"
